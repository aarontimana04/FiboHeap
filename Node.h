...
struct Node {
    T data;
    Node<T>* left, right; //Es la simulacion de lista doblemente enlazada
    Node<T>* parent; //Tiene un nodo padre en caso de que sea key es null
    Node<T>*child; //Tiene hijos
    int degree{}; //Tiene un ranking en el cual estp indica el numero de hijo

    Node(){}
    Node(T data):data(data), left(nullptr), right(nullptr), parent(nullptr), child(nullptr){}
